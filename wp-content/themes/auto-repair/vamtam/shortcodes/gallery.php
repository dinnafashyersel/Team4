	...
	public static function gallery_lightbox( $attr ) {
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			if ( ! empty( $image_src ) && ! empty( $image_src[0] ) ) {
				$output .= '<a href="' . esc_url( $image_src[0] ) . '" title="' . esc_attr__( 'View Gallery Item', 'auto-repair' ) . '" class="cbp-lightbox vamtam-lightbox-gallery"  data-title="' . esc_attr( get_the_title() ) . '"></a>';
			}
		}
